...
const TruebitCheck= ({ veevaChechsum, sigmaChecksum, selectedTab})=>{
    ...
    const truebitcheck = async () => {
        try {
          setLoader(true);
          if (!veevaChechsum || !sigmaChecksum) {
            console.log('Document details are not available.');
            return;
          }
    
          console.log("checkingre", sigmaChecksum, veevaChechsum);
    
          const checksum1 = sigmaChecksum;
          const checksum2 = veevaChechsum;
    ...
    useEffect(() => {
        ...
        if (sigmaChecksum && veevaChechsum) {
            fun();
        }
      }, [selectedTab]);
    ...
    function timestampToEpoch(timestamp) {
      ...
      return ( 
        <div>
            ...
            {/* New Container Box */}
            <br/>
            <div className="container border p-3" style={{ borderColor: "black"  }}>
            <div className="row align-items-center">
                {/* Left Corner - Checksums */}
                <div className="col-auto">
                    <p className="fontcustom">Sigmachecksum: &nbsp;&nbsp;{sigmaChecksum}</p>
                    <p className="fontcustom">Veevachecksum: &nbsp;&nbsp;{veevaChechsum}</p>
                </div>
                ...
                <div className="col-auto text-end">
                    <ButtonLoad className="btn" variant="dark" loading={loader} onClick={truebitcheck}>Verify</ButtonLoad>
                </div>
            </div>
        </div>
